...
> 完整性边界由 **Zama threshold KMS 的密码学签名** 守护，不依赖 Executor 进程或任何调用者身份的诚实性。
...
| **Executor (system)** | 消费 Gateway 解密后 handle 的链下服务 | 调 `callbackFinalize(bool, bytes)` / `executeTransfer(addr, uint64, bytes)`。KMS threshold 签名链上校验。**不是用户角色** |
...
ZamaDrop 不依赖 admin / auditor / executor 任何账户的诚实性来保证关键状态正确。所有真值由 **Zama threshold KMS** 签名背书：
...
**为什么 permissionless caller 没问题**：KMS 签名校验后，调用者唯一能表达的"权力"是 *是否中继被签的结果*。他们无法伪造 bool、无法伪造签名、无法在合法签名存在后阻止链上效果。任何运行 executor 的人（包括 recipient、admin、第三方 bot）都能 finalize campaign —— 但没人能颠覆结果。
...
### 3.4 链下 Executor 仅做 liveness，不做 integrity

**含义**：`finalize → Gateway publicDecrypt → callbackFinalize` 和 `claim → Gateway publicDecrypt → executeTransfer` 两条路径都依赖一个链下进程注意到 emitted handle、解密、提交后续交易。参考实现是 [`scripts/executor.ts`](../scripts/executor.ts) 守护进程（每 8s 轮询）；端到端 driver 在 [`scripts/cli-setup.ts`](../scripts/cli-setup.ts)。

**信任姿态**：executor **不被信任** integrity。恶意 executor 因为 §3.1 / §3.2 的 KMS 签名校验，无法伪造 bool / amount。executor 唯一能控制的是 *liveness* —— settlement 是否及时发生。

**失败模式**：离线或被审查的 executor 导致结算无限期 stall。缓解：任何人可以并行跑 executor。链上 `finalized` flag 和 `transferred[user]` map 让并行执行幂等 —— 只有第一次有效调用胜出。Recipient 可以自己跑 `scripts/executor.ts` 自助结算如果 operator 的 executor 离线。
...
[3] 链下 Executor：监听事件 → instance.publicDecrypt([h])     ← 仍是 handle，还不是明文
[4] KMS Gateway：threshold MPC 解密 → 返回 (amount, sig)        ← 在 KMS 内部明文化
[5] Executor 进程内存：amount + sig                           ← ⚠ plaintext 第一次出现
[6] Executor 调 executeTransfer(recipient, amount, sig)        ← ⚠ 写进 calldata，永久公开
[7] 合约：FHE.checkSignatures(...)                             ← KMS 签名链上校验
        token.transfer(recipient, amount)                     ← ERC20 Transfer event 公开
```

**plaintext 第一次出现是 [5]**——Executor 进程内存。**[6][7] 之后明文已链上**。

Executor 比"链上"早大约 **一个区块时间** 看到这个数。仅此而已 —— 而且这个"先看到"在 ZamaDrop 设计语义里 **没有任何特权信息**：不是 Executor 看到了 amount，是 *任何监听 Etherscan 的人* 几秒后都会看到 amount。

---

## 5. Threat Analysis

### 5.1 Executor compromise

| 攻击 | 是否成功 | 原因 |
|---|---|---|
| Executor 解密未 claim 的 allocation | ❌ | 合约从未调 `makePubliclyDecryptable` 在那些 handle 上；KMS 拒绝解密 |
| Executor 解密别的 campaign 数据 | ❌ | 每个 handle 绑定具体 contract address，KMS 校验 |
| Executor 解密 `_runningTotal` | ❌ | 从未在 ACL 里、从未被标记 publicly decryptable |
| Executor 解密 `_claimedTotal` | ❌ | 仅 auditor 在 ACL |
| Executor 抢先看到 *claim 后即将公开* 的 amount | ✅ 部分 | 但同样数据 ~10s 后链上即公开，仅时序优势 |
| Executor 拒绝服务（不 settle 任何 claim） | ✅ | 缓解：任何人可以跑一份新 executor 兜底 |
| Executor 伪造 amount 转账 | ❌ | KMS 签名 + `FHE.checkSignatures`，伪造金额 revert |
| Executor 抢跑 callbackFinalize 翻 bool | ❌ | 同上 |
...
这两处 gap **和 Executor 是 UI 还是 backend 无关**。哪怕完全不做 Executor、让 recipient 自己在前端 publicDecrypt 然后自己调 callback —— 攻击向量依然存在。Executor 不是信任根；KMS 才是。
...
- **Executor 不再是信任根** —— 任何人都可以充当 Executor，包括 recipient 自己、admin 自己、第三方机器人
- Executor 哪怕完全恶意，也无法伪造 amount / bool —— 合约会拒收
- "Executor" 退化为 *无状态自动化*，仅做事件监听 → KMS 调用 → 透传签名
...
| Executor 进程独立、可由任何人运行 | ✅ | ✅ |
| Executor 进程是信任根 | ⚠ MVP 信任 | ❌ KMS 是信任根 |
...
### 8.2 引入显式 Executor 角色 · ✗ 故意不做

§8.1 落地后，显式 `executor` 角色不再必要：`executeTransfer` 调用者无法影响 amount / recipient / claim 状态，限制为 keeper 只是减少 liveness 冗余。任何人可以跑 [`scripts/executor.ts`](../scripts/executor.ts) 兜底 —— 多并行运行通过链上 `transferred[user]` map 幂等。
...
- **侧信道**：Executor 进程内存中短暂持有 plaintext。运维侧应禁用 swap、限制 core dump、不打 plaintext 到日志
