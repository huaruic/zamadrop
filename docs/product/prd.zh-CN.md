...
### 6.1 已落地的 production 加固（不在初版 MVP scope，但实际已交付）

- **KMS threshold 签名校验**：`callbackFinalize(bool, bytes decryptionProof)` 与 `executeTransfer(address, uint64, bytes decryptionProof)` 在合约里通过 `FHE.checkSignatures` 校验 Zama threshold KMS 签名后才能改写状态。这关闭了之前公开记录的"任何调用者均可任意翻 finalized / 任意金额执行转账"的 MVP gap。
- **链下 executor 守护进程**：[`scripts/executor.ts`](./scripts/executor.ts) 自动监听 `FinalizeRequested` / `ClaimRequested` 事件、调 KMS publicDecrypt、回写合约。**信任根是 KMS 签名，不是 executor 进程**——任何账户都可以跑这份脚本兜底。
- **测试覆盖**：`test/ZamaDropCampaign.test.ts` 26 passing，包括 "amount 与 KMS 解密结果不一致时应 revert（防伪造）"。
- 完整说明：[`SECURITY.md`](./SECURITY.md) §2.2、§3、§5、§6。
...
Landing Page 已搬到独立仓库 `secret-drop`，作为产品营销面（不接钱包，CTA 跳到 `app.zamadrop.xyz` 子域）。dApp 仓库不再维护 landing 视觉规范；设计 token 通过 `frontend/src/styles/tokens.css` 与 `frontend/src/styles/effects.css` 与 landing 仓共享，保证视觉一致。
