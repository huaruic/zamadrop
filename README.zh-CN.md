...
**基于 Zama fhEVM 的机密代币分发协议 —— 加密每个受益人的分配金额，并在密文域内证明 campaign 总额。**
...
🚀 为 **Zama Protocol Bounty** 而建 —— Confidential Onchain Finance 赛道。

</div>

---

> **一句话** —— 公开空投会泄露一份高精度的目标定位数据库:任何人都能按金额给受益人排序。ZamaDrop 让 campaign 级事实(总额、受益人数、领取进度)保持完全公开可验证,而每个受益人的金额以 FHE 密文形式上链,只有本人能解密。
...
- 🔐 **FHE 加密分配** —— 每个受益人的金额以 `euint64` 密文形式上链,只有受益人本人能解密。
- 🧮 **密文域总额校验** —— 合约*完全在密文状态下*校验 `sum(allocations) == declaredTotal`,项目方无法在密文掩护下偷偷克扣总额。
- 👁️ **公开可问责** —— 声明总量、受益人数、finalize 状态、领取进度全部保持完全公开可验证。
- 🛡️ **KMS 加固结算** —— `callbackFinalize` 与 `executeTransfer` 在改写状态前先用 `FHE.checkSignatures` 校验 Zama threshold KMS 签名;伪造的 bool 或金额会 revert。
- 🧑‍⚖️ **内建 auditor 角色** —— 指定的 auditor 可解密聚合的 `claimedTotal` 用于合规报告,但永远看不到任何单笔金额。
- 🌐 **无需链下 daemon (V7+)** —— 浏览器内 active-pull:触发流程的钱包从 Gateway 拿到 KMS 签名后自己提交回调,端到端约 10–15 秒。

## 🔥 亮点

> **最新进展 —— 当前版本已完成 KMS 加固并通过 Sepolia 验证。**

- 🎉 **[V7] 浏览器内 active-pull KMS 回调** —— 链下 executor 守护进程已删除。触发流程的钱包向 Zama Gateway 请求 threshold-MPC 签名,然后自己提交 `callbackFinalize` / `executeTransfer`。详见 [ADR 0003](./docs/ADR/0003-frontend-as-primary-executor.md)。
- 🛡️ **[KMS 加固] threshold 签名把关的结算** —— 两个改写状态的回调现在都先跑 `FHE.checkSignatures`,关闭了早期 MVP 版本记录在案的完整性 gap。
- 🌐 **[Sepolia] 线上 KMS 加固部署** —— 当前合约部署在 Ethereum Sepolia 测试网;事实来源见 [`deployments/sepolia.json`](./deployments/sepolia.json)。
- 🧪 **[测试] 26 个 Hardhat 测试通过 + 真实 MetaMask E2E** —— fhEVM mock 单元套件,外加 Synpress + Playwright 钱包回归覆盖。
...
ZamaDrop 用 Zama 的全同态加密把这两层一直被绑在一起的设计解耦:

| 层 | 可见性 | 内容 |
|---|---|---|
| 🟢 **Campaign 级事实** | 公开可验证 | 声明总量、受益人数、finalize 状态、领取进度 |
| 🔴 **单笔分配金额** | 加密 | `euint64` 密文,只有受益人本人能解密 |
| 🟡 **聚合指标** | 仅 auditor | `claimedTotal` —— 由 auditor 角色解密,永远看不到单笔金额 |

合约仍然在密文状态下校验总量约束 —— `sum(allocations) == declaredTotal` —— 项目方无法在密文掩护下偷偷克扣总额。

> **V7 隐私边界** —— 接收者的*成员身份*在设计上是链上公开的:资格列表不再泄露金额,但接收者集合本身可由合约事件枚举。完整隐私边界见 [`docs/SECURITY.md`](./docs/SECURITY.md#v7-privacy-boundary)。
...
- 👮 **Admin** 声明总量、为每个受益人设置加密 allocation、触发 `finalize`,并(V7+)通过 relayer SDK 主动 pull KMS 签名后自己提交 `callbackFinalize`。无需任何链下 daemon。
- 🎁 **Recipient** 在浏览器端通过 re-encryption 解密自己的 allocation,签 `claim`,再 pull per-claim KMS 签名后自己提交 `executeTransfer`。两次钱包签名,端到端约 15 秒。
- 🧑‍⚖️ **Auditor** 解密聚合的 `claimedTotal` 用于合规报告 —— 永远看不到任何单笔金额。
- 👁️ **Public** 不需要钱包即可读取 campaign 元数据与 finalize 状态。

合约通过 `FHE.checkSignatures` 在改写状态前校验 KMS threshold 签名,因此 caller 身份对完整性无关 —— **信任根是 KMS 签名而不是谁在提交**。架构理由见 [ADR 0003](./docs/ADR/0003-frontend-as-primary-executor.md),完整信任分析见 [安全模型](#-安全模型)。
...
### KMS 回调(V7+:浏览器内 active-pull)

V7 删掉了链下 executor 守护进程。共享 util [`frontend/src/lib/kms-active-pull.ts`](./frontend/src/lib/kms-active-pull.ts) 提供 `pullAndCallbackFinalize` 与 `pullAndExecuteTransfer`:触发流程的同一钱包通过 `relayer-sdk.publicDecrypt` 主动从 Zama Gateway 拿到 threshold-MPC 签名,然后自己提交 `callbackFinalize`(admin)或 `executeTransfer`(recipient)。Gateway 健康时端到端约 10–15 秒。信任根是 `FHE.checkSignatures` 链上校验的 KMS 签名,caller 身份无关。详见 [ADR 0003](./docs/ADR/0003-frontend-as-primary-executor.md)。

CLI 救援场景:[`scripts/recover-stuck-finalize.ts`](./scripts/recover-stuck-finalize.ts) 用 `hre.fhevm.publicDecrypt` 演示同一模式 —— 当 campaign 卡在 Finalizing 状态而 UI 客户端还没运行时使用。

运维辅助脚本:`scripts/verify-roles.ts`(核对 admin / auditor + 最近 allocation 事件),`scripts/verify-decrypts.ts`(针对已部署合约重放公开解密),`scripts/cli-setup.ts`(fresh 部署的 E2E 驱动 —— 内部也走 active-pull 模式)。
...
├── scripts/                # recover-stuck-finalize(CLI 救援)、verify-roles、verify-decrypts、cli-setup、e2e-sepolia
...
- ✅ **v0.x(当前)** —— 四角色 MVP,KMS 加固结算已上线,Sepolia 已验证,真实 MetaMask E2E 覆盖。V7 上线浏览器内 active-pull KMS 回调(无需链下 daemon)+ admin 友好的 5 步部署 wizard。
- 🚧 **v1** —— auditor 多签;Merkle 资格集成,使 ZamaDrop 干净地叠加在现有 Merkle 空投工具链之上;生产部署中分离 admin / auditor 钱包;V8 finalize-recovery escape hatch 处理长时间 Gateway 不可用。
...
基于 [Zama fhEVM](https://docs.zama.ai/fhevm) 构建 · 提交至 [Zama Protocol Bounty](https://www.zama.ai/)
